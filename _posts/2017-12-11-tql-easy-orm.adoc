...
This will select the first 10 `TodoItem`s, sorted from the newest date to the latest and select only the items where the field `done` is equal to `false`.
...
However, this is not a big deal, because you can easily develop your project on nightly, and then switch to stable for your deployment since you're not supposed to have compiler errors when you're ready to deploy.
...
Take not that `tql` is not ready for production use, it's alpha quality and not thoroughly tested (especially the stable version), so use it at your own risk.
...
(Don't be afraid my the size of the report, it contains the whole code of `tql`.)
