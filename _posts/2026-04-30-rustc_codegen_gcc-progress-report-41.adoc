...
I had some trouble again with the, so we'll try to run more tests in the CI of the Rust repo in order to prevent having to fix issues multiple times since they can come back when changes are made in the Rust repo.
...
The plan is to compile the standard library and run the tests of libcore in the CI of the Rust repo which is one of the few tests we currently don't run there.
